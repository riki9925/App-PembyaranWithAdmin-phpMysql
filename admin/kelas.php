...
<div id="content">
	...
	<form name="form1" method="post" action="">
	  <table width="300" border="0" class="table-form">
		...
		<tr>
          <td>Biaya SPP</td>
          <td><input name="spp" type="numeric" id="spp" /></td>
        </tr>
		   <td>&nbsp;</td>
	...
	<?php
		...
		$spp=$_POST['spp'];
		if(isset($id_kelas,$kelas,$spp)){
			if((!$id_kelas)||(!$kelas)||(!$spp)){
			print "<script>alert ('Harap semua data diisi...!!');</script>";
			print"<script> self.history.back('Gagal Menyimpan');</script>";	
			exit();
			}	
		$add_kelas="INSERT INTO kelas(id_kelas,kelas,spp) VALUES ('$id_kelas','$kelas','$spp')";
		...
		$show_kelas="SELECT * FROM kelas ORDER BY kelas ASC LIMIT $offset, $rowsPerPage";
		...
		while($view_kelas=mysql_fetch_row($hasil_kelas)){
			print("<tr>
				...
				if($pilih_user[status]==admin){
			print("
				<td class=table-common-links><a href=editkelas.php?idk=$view_kelas[0]>Edit</a>
